...
class AlignmentBlock:
    ...
    def call_intra_chain_indels(self):
        """
        Parses the indel_map to yield intra-chain indels.
        Returns a list of dictionaries containing the variant details.
        CURRENT IN A STATE OF DISREPAIR< THIS NEEDS DEBUGGING BEFORE USE
        """
        
        if not self.indel_map:
            return []

        # Always read reference left-to-right
        ref_pos = self.left_most 
        
        # Query direction depends on the strand
        is_reverse = self.slope < 0
        qry_pos = self.query_end if is_reverse else self.query_start
        qry_step = -1 if is_reverse else 1

        variants = []
        
        # State trackers for consolidating contiguous indels
        current_svtype = None
        ...
        for d in self.indel_map:
            ...
            # 1. Advance through the exact matches
            ref_pos += matches
            qry_pos += (matches * qry_step)
            
            # 2. Determine the indel type at this location
            svtype = "DEL" if d > 0 else "INS"
            
            # 3. Check if this is a new variant or contiguous with the last one
            if matches > 0 or current_svtype != svtype:
                # Save the previously tracked variant if it exists
                if current_svtype is not None:
                    variants.append({
                        "chrom": self.reference,
                        "pos": current_ref_pos,
                        "svtype": current_svtype,
                        "svlen": current_length,
                        "end": current_ref_pos + (current_length if current_svtype == "DEL" else 0)
                    })
                
                # Start tracking the new variant
                current_svtype = svtype
                current_ref_pos = ref_pos
                current_length = 1
            else:
                # Contiguous indel (e.g., a chain of -1s), just extend the length
                current_length += 1
                
            # 4. Advance the coordinate past the indel base itself
            if d > 0:
                ref_pos += 1          # Deletion: ref advances, query stalls
            else:
                qry_pos += qry_step   # Insertion: query advances, ref stalls

        # Append the tracked variant (Update both instances in the method)
        if current_svtype is not None:
            variants.append({
                "chrom": self.reference,
                "pos": current_ref_pos,
